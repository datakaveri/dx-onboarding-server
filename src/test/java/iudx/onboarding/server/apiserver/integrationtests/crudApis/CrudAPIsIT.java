package iudx.onboarding.server.apiserver.integrationtests.crudApis;
...
public class CrudAPIsIT {
  ...
  void createDXResourceGroupItemEntity() {
    ...
    if (!itemExists("bbeacb12-5e54-339d-92e0-d8e063b551a8")) {
...
//      .body("results.item_details.id", notNullValue())
      .extract()
      .response();

    LOGGER.debug(response.body().toString());
    LOGGER.debug("" + response.path("results"));
    LOGGER.debug("" + response.path("results.item_details"));
    LOGGER.debug("" + response.path("results.item_details.id"));
    // Extract the generated ID from the response
    resource_group_id = response.path("results.item_details.id");
  }
  ...
  void createDXResourceItem() {
    LOGGER.debug("from DX RS item..."+resource_server_id +" "+"bbeacb12-5e54-339d-92e0-d8e063b551a8"+" "+resource_group_id);
    ...
    if (!itemExists("49cf5c76-09de-11ee-be56-0242ac120002")) {
    ...
    JsonObject jsonPayload = new JsonObject()
      ...
      .put("resourceServer", "49cf5c76-09de-11ee-be56-0242ac120002")
      .put("provider", "bbeacb12-5e54-339d-92e0-d8e063b551a8")
  ...
  void createDXEntityInvalidCredentials() {
    JsonObject jsonPayload = new JsonObject()
      ...
      .put("owner", "cc18c925-8990-3a07-9cac-0bb2bd8166f6")
